...
- **Locked state — BOTH universe AND character lock once a convo has messages** (immersion: a chat is *with one persona* — §1 "the persona is the product"; swapping mid-thread breaks it). Both chips go flat (drop shadow), `--ink-muted`, small lock glyph, with tooltips ("Start a new chat to change universe" / "…to talk to someone else"). The character is freely changeable **only in the empty state** (that's the "pick your sensei" moment, which re-sequences); to switch once talking, start a New Chat. (The old dark chat locked only the universe — that was a flaw, not a feature.)
...
- [~] **3. Thread + composer** — split into 3a/3b for reviewability.
  ...
  - [x] **3b. Composer** — ✅ DONE. `Composer.tsx` + `cast.ts` (static CAST/UNIVERSES, mirrors SenseiRail). Sharp paper-raised panel, `3px` ink top border (no blur/round/accent). Auto-grow textarea, persona placeholder `Message {name}…`, Enter-to-send / Shift+Enter newline. **Universe chip** = Space Mono tag (globe + name + chevron); **locked state** when a convo has messages (flat, `--ink-muted`, lock glyph, tooltip). **Character chip** = avatar folded in + Clash name. **Popovers open upward** (sharp, ink border, hard shadow, click-away catcher): universe = WORLDS list (selected red left-bar); character = cast cards mirroring SenseiRail (avatar + Clash name + `universe · trait`, selected red bar + ghost). **Red Send** (square, lift) + `send` SFX (WHOOSH) on click; **`impact` SFX (DON)** on character-select. Wired into `ChatPreview`: selected-character state drives header/thread/placeholder; **send loop** = append user bubble → typing dots (750ms) → stream a canned reply (Step 3b placeholder; Step 5 = pools) with the inline `▍` caret. Verified desktop + mobile (375, no h-overflow): both popovers, locked/unlocked universe, send→stream, `tsc`/ESLint clean.
    ...
    - **Character lock (review):** the character chip now also locks once a convo has messages (`isCharacterLocked` = same `messages.length > 0` condition as universe) — see the Composer spec above. Changeable only in the empty state.
- [x] **4. Empty state** — ✅ DONE. `EmptyState.tsx` + `persona.ts` (per-character greeting + per-mode prompt pools; Naruto full, others lighter, generic fallback; `{user}` personalization; `sample()` for variety). Sequence on character-select: **identity** (big square avatar + speed-lines + Clash name + Space Mono `universe · trait`) → **in-voice greeting auto-types** into a left `Bubble`+avatar set (red `*actions*`, `▍` caret, personalized) → **VN-style dialogue-choice prompts reveal** (right-aligned, red `▸`, hard-shadow lift). **EXPLAIN/HYPE ME/JUST TALK** mode tags swap the prompt set; **RE-ROLL** re-samples + `impact` SFX (DON). Prompts **dim** (opacity-30, pointer-events-none) while the composer has a draft (`onDraftChange`→`draft`); **tapping a prompt sends** it (WHOOSH + transition to thread). Verified desktop + mobile (375): full sequence, mode switch, typing-dim, prompt-tap→thread, **character-switch re-sequences** (Sasuke identity/greeting/prompts), no h-overflow, `tsc`/ESLint clean.
- [x] **5. Semi-interactive demo** — ✅ DONE. `replies.ts`: `TAILORED` map (each Naruto prompt → on-topic in-voice answer, exact-string keyed to `persona.ts`; 2 carry the full markdown showcase — bullet list + bold + `code` block + action) + `FREE_POOL` per character (Naruto rich, others lighter) + `GENERIC_POOL` fallback; `replyFor(character, text)` returns the tailored answer for a known prompt else a sampled pool reply. `ChatPreview.handleSend` now computes `replyFor` and streams it after **600–900ms** of ink-square typing dots. Verified: tailored prompt → markdown reply, free-typed → sampled in-voice reply (varies), typing dots (`brutal-march` ×3) fire in the delay window, inline streaming caret. `tsc`/ESLint clean.
- [x] **6. Modals** — ✅ DONE. `Modal.tsx` (reusable: `fixed` overlay, **solid-ink no-blur backdrop**, hard `3px` ink paper panel + `7·7·0` shadow, snappy `animate-in zoom-in-95` pop, corner ✕, Esc + backdrop-click close). `Modals.tsx` = `SearchModal` (⌘K / Ctrl+K toggle + header search button; Space Mono placeholder + ⌘K badge with right-padding to clear the ✕; RECENT CHATS flat rows → select+close; Enter-on-empty → New Chat) + `ProfileModal` (square red avatar, Clash name, email, FREE PLAN tag, secondary SIGN OUT `Button`). Wired in `ChatPreview` (`searchOpen`/`profileOpen`, ⌘K listener); Sidebar user card → `onProfile`. Verified desktop + mobile (375, no h-overflow): ⌘K opens search, user card opens profile, Esc/backdrop close.
  - **Remaining mobile polish (deferred/minor):** picker popovers are upward dropdowns (fine at 375, no bottom-sheet yet); message hover-actions are hover-only (tap-hold not added) — both low-priority follow-ups, not blockers.

### Status — preview COMPLETE
All 6 build steps done + verified end-to-end (desktop + mobile 375, no h-overflow, no chat-brutal console errors, `tsc`/ESLint clean). Lives at **`/preview/chat`**; the real dark `/chat` is untouched. Sidebar **tone CONFIRMED ink** (judged at full size — user likes it; the `paper` variant stays in `TONE_VARS` as a documented fallback). Deferred non-blockers: picker bottom-sheet on mobile + tap-hold message actions. Next phase (not now) = wiring to real APIs + the go-live swap (landing + chat + auth together), per the "Wiring to real APIs later" notes above.
